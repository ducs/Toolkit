...
using Toolkit.Modularity.Common;
    ...
    public class ModuleDependencySolver
    {
        ...
        public void AddModule(string name)
        {
            if (String.IsNullOrEmpty(name))
                throw new ArgumentException($"StringCannotBeNullOrEmpty:{name}");
        ...
        public void AddDependency(string dependingModule, string dependentModule)
        {
            if (String.IsNullOrEmpty(dependingModule))
                throw new ArgumentException($"StringCannotBeNullOrEmpty:{dependingModule}");

            if (String.IsNullOrEmpty(dependentModule))
                throw new ArgumentException($"StringCannotBeNullOrEmpty:{dependentModule}");

            if (!knownModules.Contains(dependingModule))
                throw new ArgumentException($"Resources.DependencyForUnknownModule:{dependingModule}");
        ...
        public string[] Solve()
        {
            ...
            if (skip.Count > knownModules.Count)
            {
                ...
                throw new ModularityException(moduleNames,
                    $"Resources.DependencyOnMissingModule:{moduleNames}");
            }
